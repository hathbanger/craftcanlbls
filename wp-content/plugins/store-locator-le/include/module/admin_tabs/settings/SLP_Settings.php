...
if ( ! class_exists( 'SLP_Settings' ) ) {
	...
	class SLP_Settings extends SLPlus_BaseClass_Object {
		...
		function add_section( $params ) {
			...
			if ( ! isset( $params['name'] ) ) {
				...
				if ( ! isset( $params['slug'] ) ) {
					...
					$params['name'] = $this->slplus->text_manager->get_text_string( array(
						'settings_section',
						$params['slug'],
					) );
				}
		...
		private function add_smart_options_to_section( $section ) {
			if ( empty( $this->slplus->SmartOptions->page_layout[ $this->slplus->current_admin_page ] ) ) {
				return;
			}
			if ( empty( $this->slplus->SmartOptions->page_layout[ $this->slplus->current_admin_page ][ $section ] ) ) {
				return;
			}
			...
			foreach ( $this->slplus->SmartOptions->page_layout[ $this->slplus->current_admin_page ][ $section ] as $group => $properties ) {
		...
		function add_group( $params ) {
			...
			if ( ! isset( $params['header'] ) ) {
				$params['header'] = $this->slplus->text_manager->get_text_string( array(
					'settings_group',
					$params['group_slug'],
				) );
			}
		...
		private function add_help_section() {
			...
			return
				...
				'<h3 class="panel-content aside-heading">' . $this->slplus->text_manager->get_text_string( array(
					'admin',
					'help_header',
				) ) . '</h3>' .
				'<div class="panel-content">' .
				'<h4 class="description-heading">' . $this->slplus->text_manager->get_text_string( array(
					'admin',
					'help_subheader',
				) ) . '</h4>' .
