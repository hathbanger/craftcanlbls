...
if ( ! class_exists( 'SLP_Admin_Experience' ) ) {
	...
	class SLP_Admin_Experience extends SLP_Object_With_Objects {
		...
		function add_map() {
			 ...
			 */
			do_action( 'slp_ux_modify_adminpanel_' . $slug , $this->Settings, $this->Settings->get_section( $slug )->name , $this->Settings->get_section( $slug )->get_params() );
		}
		...
		function add_view() {
			 ...
			 * @param   array $section_params ['name'] = string of name, ['slug'] = string of unique slug
			 *
			 * TODO: remove this when PRO uses the slp_build_map_settings_panels filter instead.
			 */
			do_action( 'slp_uxsettings_modify_viewpanel', $this->Settings, $this->Settings->get_section( $slug )->name , $this->Settings->get_section( $slug )->get_params() );
		}
		...
		function display() {
			...
			if ( ! ( strpos( $this->slplus->smart_options->map_home_icon, 'http' ) === 0 ) ) {
				$this->slplus->smart_options->map_home_icon = $this->slplus->data['siteURL'] . $this->slplus->smart_options->map_home_icon;
			}
			if ( ! ( strpos( $this->slplus->smart_options->map_end_icon, 'http' ) === 0 ) ) {
				$this->slplus->smart_options->map_end_icon = $this->slplus->data['siteURL'] . $this->slplus->smart_options->map_end_icon;
			}
			if ( ! $this->slplus->Helper->webItemExists( $this->slplus->smart_options->map_home_icon ) ) {
				$this->slplus->data['iconNotice'] .=
					sprintf(
						__( 'Your home marker %s cannot be located, please select a new one.', 'store-locator-le' ),
						$this->slplus->smart_options->map_home_icon
					)
					.
					'<br/>';
			}
			if ( ! $this->slplus->Helper->webItemExists( $this->slplus->smart_options->map_end_icon ) ) {
				$this->slplus->data['iconNotice'] .=
					sprintf(
						__( 'Your destination marker %s cannot be located, please select a new one.', 'store-locator-le' ),
						$this->slplus->smart_options->map_end_icon
					)
					.
					'<br/>';
			}
		...
		function save_options() {
			...
			// Checkboxes
			//
			// TODO: Update EM to remove slp_save_map_settings_checkboxes then this can go away.
			//
			...
			$this->slplus->smart_options->execute_change_callbacks();       // Anything changed?  Execute their callbacks.
			$this->slplus->option_manager->update_wp_option( 'js' );        // Change callbacks may interact with JS or NOJS, make sure both are saved after ALL callbacks
		 ...
		 **/
		private function SaveCheckboxToDB($boxname,$prefix = null, $separator='-') {
