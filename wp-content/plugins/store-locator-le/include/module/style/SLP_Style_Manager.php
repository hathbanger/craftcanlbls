<?php
if ( ! class_exists( 'SLP_Style_Manager' ) ) {
	/**
	...
	class SLP_Style_Manager extends SLPlus_BaseClass_Object {
		...
		public function fetch_style( $style_id = null ) {
			require_once( SLPLUS_PLUGINDIR . 'include/module/internet/SLP_Internet_Helper.php' );

			$style_selector = is_null( $style_id ) ? '' : sprintf( '/%d' , $style_id );
			$request_params = '?orderby=title&order=asc&per_page=' . $this->page_size;
			// $response = $this->slplus->Internet_Helper->get_and_validate_json_response( 'http://local.wordpress.dev/' . SLP_Style_Manager::REST_ENDPOINT . $style_selector );
			$response = $this->slplus->Internet_Helper->get_and_validate_json_response( SLPLUS::url_services . SLP_Style_Manager::REST_ENDPOINT . $style_selector . $request_params );
		...
		private function style_matches_slug( $slug ) {
			return ( $slug === $this->slplus->smart_options->style->value );
		}
	}
	...
	if ( is_a( $slplus , 'SLPlus' ) ) {
		$slplus->add_object( new SLP_Style_Manager() );
	}
}